...
Matrix::Matrix(const std::size_t n_rows, const std::size_t n_cols)
    : n_rows_(n_rows), n_cols_(n_cols), storage_(new double[n_rows * n_cols]()) {}

Matrix::~Matrix() { delete[] storage_; }

double& Matrix::operator()(const std::size_t row, const std::size_t col) {
  if (row >= n_rows_ || col >= n_cols_) throw std::out_of_range("Matrix index out of range.");
